...
export const getUserOrders = asyncHandler(async (req, res) => {
  const orders = await Order.find({ user: req.user._id })
    ...
    .populate(
      "items.product",
      "name price images description brand category color"
    )
    .sort({ createdAt: -1 });
...
export const getOrderById = asyncHandler(async (req, res, next) => {
  const order = await Order.findById(req.params.id)
    ...
    .populate({
      path: "items.product",
      select: "name price images description category brand variants",
      populate: [
  ...
  const orderObj = getLocalizedResponse(req, order);
  orderObj.customer = getUserByOrder(order);
  delete orderObj.user;

  // Only add color info to each item, and remove all variant data from product
  orderObj.items = orderObj.items.map((item) => {
    ...
    if (item.product && item.product.variants) {
      for (const variant of item.product.variants) {
        ...
        if (matchedOption) {
          colorInfo = {
            colorName: matchedOption.colorName,
            colorHex: matchedOption.colorHex,
            colorSwatchImage: matchedOption.colorSwatchImage,
          };
          break;
        }
      }
    ...
    return {
      ...
      colorSwatchImage: colorInfo.colorSwatchImage,
    };
  });

  res.status(200).json({ status: "success", data: { order: orderObj } });
});
// export const getOrderById = asyncHandler(async (req, res, next) => {
//   const order = await Order.findById(req.params.id)
//     .populate("user", "firstName lastName email phoneNumber profilePicture")
